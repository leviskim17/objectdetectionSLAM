...
class GUI {
 public:
  GUI(bool liveCap, bool showcaseMode) : showcaseMode(showcaseMode) {
    ...
    s_cam = pangolin::OpenGlRenderState(pangolin::ProjectionMatrix(screenWidth, screenHeight, 420, 420, screenWidth / 2.0f, screenHeight / 2.0f, 0.1, 1000),
                                        pangolin::ModelViewLookAt(0, 0, -1, 0, 0, 1, pangolin::AxisNegY));

    pangolin::Display("cam").SetBounds(0, 1.0f, 0, 1.0f, -screenWidth / screenHeight).SetHandler(new pangolin::Handler3D(s_cam));

    pangolin::Display(GPUTexture::RGB).SetAspect(screenWidth / screenHeight);
    pangolin::Display(GPUTexture::DEPTH_NORM).SetAspect(screenWidth / screenHeight);
    pangolin::Display(GPUTexture::MASK_COLOR).SetAspect(screenWidth / screenHeight);
    pangolin::Display("ModelImg").SetAspect(screenWidth / screenHeight);

    pangolin::Display("ICP1").SetAspect(screenWidth / screenHeight);
    pangolin::Display("ICP2").SetAspect(screenWidth / screenHeight);
    pangolin::Display("ICP3").SetAspect(screenWidth / screenHeight);
    pangolin::Display("ICP4").SetAspect(screenWidth / screenHeight);
    ...
    if (!showcaseMode) {
      ...
      pangolin::CreatePanel("oi").SetBounds(0.0, 1.0, 0.0, pangolin::Attach::Pix(widthPanel));
      pangolin::CreatePanel("ui").SetBounds(0.0, 1.0, 0.88, 1.0);
  ...
  void addModel(int id, float confThres) 
  { 
    modelInfos.emplace_back(id, confThres); 
  }

  void saveColorImage(const std::string& path) 
  { 
    pangolin::SaveWindowOnRender(path); 
  }
